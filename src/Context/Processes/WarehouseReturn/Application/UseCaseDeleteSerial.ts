...
export class UseCaseDeleteSerial {
    ...
    async exec(_id: string, { keyDetail, serial }: { keyDetail: string, serial: string }) {
        ...
        const warehouseStockSerial = await this.searchEmployeeStockSerial(warehouseStock, serial)
    ...
    private async searchEmployeeStockSerial(employeeStock: EmployeeStockENTITY, serial: string) {
        const pipeline = [{
            $match: {
        ...
        if (employeeStockSerial.state !== StateStockSerialEmployee.RESERVADO) {
            throw new BadRequestException(
